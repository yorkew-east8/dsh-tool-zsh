...
/**
* Generic-task adaptation for background zsh process handles.
*
* @module @deepseek-ai/dsh-tool-zsh/background
*/
/**
...
/**
* Model-facing result rendering for the zsh tool.
*
* @module @deepseek-ai/dsh-tool-zsh/render
*/
...
* @module @deepseek-ai/dsh-tool-zsh
*/
const name = "tool-zsh";
const inject = [
	"tools",
...
/** Runtime configuration schema for the zsh tool plugin. */
const Config = z.object({ enableRunInBackground: z.boolean().default(true) });
function validateZshArgs(args) {
...
function zshDescription(backgroundEnabled, escalationModes) {
...
*/
function presentZshResult(args, result) {
...
function canonicalZshResult(result) {
	const output = (stream) => ({
		text: stream.text,
...
/** Canonical background-handle properties shared by the zsh output union. */
...
function apply(ctx, config = {}) {
	...
	if (defaultMode !== void 0 && sandboxPolicy === void 0) throw new Error("tool-zsh: the mounted zsh executor confines but ctx.sandboxPolicy is missing");
	...
	*/
	const approveZshEscalation = (mode, justification, exec, standingPolicy) => {
	...
	ctx.systemPrompt.section({
		name: "tool:zsh",
		order: 105,
		text: "Check the [exit code: N] marker on every zsh result; investigate failures before moving on."
	});
	ctx.tools.register(defineTool({
		name: "zsh",
		description: zshDescription(backgroundEnabled, escalationModes),
		parameters: {
			command: {
				type: "string",
				required: true,
		...
		async execute(args, exec) {
			validateZshArgs(args);
			const standingPolicy = resolveSandboxPolicy(exec);
			const approvedMode = args.sandbox_permissions !== void 0 && args.justification !== void 0 ? await approveZshEscalation(args.sandbox_permissions, args.justification, exec, standingPolicy) : void 0;
			...
			return {
				kind: "foreground",
				...canonicalZshResult(result)
			};
		},
		presentCall: presentZshCall,
		presentResult: presentZshResult
	}));
}
//#endregion
